...
- **Session timeout**: a client idle watcher signs the user out after **30 minutes** of inactivity (shared across tabs). **(Supabase-managed)** Absolute expiry and refresh-token rotation are erned by Supabase JWT settings (**Authentication → Sessions / JWT expiry**); `proxy.ts` refreshes/validates the session on every request.
